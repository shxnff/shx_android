...
import com.shx.base.utils.provider.mode.SpMode;

import java.util.HashSet;
import java.util.Set;

/**
 * 功能介绍：内容提供者用到的sp,只处理{@link OgowContentProvider#HANDLE_SP}类型
 */
public class ImpSpProvider implements IContentProvider {
    ...
    boolean remove(Context context, String name, String key) {
        SharedPreferences.Editor editor = SPUtils.getSP(context, name).edit();
        editor.remove(key);
        return editor.commit();
    }

    boolean clear(Context context, String name) {
        SharedPreferences sp = SPUtils.getSP(context, name);
     ...
     * content://com.ogow.activity.OgowContentProvider/handle_sp/string/user/nickname/
     */
    @Override
